...
```  
git clone https://github.com/miyagaw61/mgtools.git $HOME/Documents/git/mgtools  
```  
...
使用するにはbashrcに  
source $HOME/Documents/git/mgtools/cdir  
を追記する必要がある。  
```  
Usage: dirmk file  
```  
...
lseコマンドを使用するためには、"source $HOME/Documents/git/mgtools/numcd.alias"の$HOME/.bashrcへの追記と$HOME/Documents/numcd/ディレクトリの作成が事前に必要。  
```  
